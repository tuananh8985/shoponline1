...
class FileUploader {
 ...
 // script để thực hiện quá trình upload
	...
	function __construct($fileTypesAllowed = 'exl, exls , doc , docx , pdf ', $fileSizeMax = 10, $uploadDir = 'img/upload/') {
		$this->fileTypesAllowed = strtolower($fileTypesAllowed);
		$this->fileSizeMax = $fileSizeMax;
	//	$shop=isset($_GET["id"])?$_GET["id"]:'';
	...
	*/
	function getFilesUpload($files) {
		...
		for ($i = 0; $i < count($files['name']); $i++) {
			...
			$this->files[$j]['ext'] = strtolower(substr($this->files[$j]['name'], strrpos($this->files[$j]['name'], '.') + 1));
				//$this->files[$j]['newName'] = $this->uploadDir.md5_file($this->files[$j]['tmp_name']).'.'.$this->files[$j]['ext'];
				$this->files[$j]['newName'] = $this->uploadDir.$this->files[$j]['name'];
				$j++;
			}
		}
	}
	...
	*/
	function getLink($i) {
		...
		$return .= "<tr><td colspan=\"2\"><a href=\"javascript:window.opener.document.contactForm.userfile.value='$link_bs';window.close();window.history.go(1);\">close</a></td> </tr></table>";
		return $return;
	}
}
?>
